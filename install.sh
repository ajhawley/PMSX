...
sudo chown -R dietpi:dietpi "/home/pi/Library/Application Support"
...
sudo chown -R dietpi:dietpi /opt/NzbDrone
...
sudo chown -R dietpi:dietpi /opt/Radarr
...
sudo chown -R dietpi:dietpi /opt/cardigann
...
sudo -u dietpi deluged
sleep 10
sudo -u dietpi deluge-console "config -s allow_remote True"
cp -i ./files/deluge-web.conf "$configpath/deluge/web.conf"
echo "pi:welcome:10" >> "$configpath/deluge/auth"
chown dietpi:dietpi "$configpath/deluge/auth"
sleep 5
pkill -9 deluged
